...
import (
	...
	v1 "github.com/razi8/trufflehog/v3/pkg/detectors/sentrytoken/v1"
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for authToken := range uniqueAuthTokens {
		s1 := detectors.Result{
			DetectorType: detector_typepb.DetectorType_SentryToken,
			Raw:          []byte(authToken),
			SecretParts:  map[string]string{"key": authToken},
		}
...
func (s Scanner) Type() detector_typepb.DetectorType {
	return detector_typepb.DetectorType_SentryToken
}
